...
function search() {
    ...
    for (let i = 0; i < total; i++) {
        ...
        <span name="fav" class="position-absolute top-0 end-0 badge rounded-pill bg-danger noprint" data-id="${items[i].id}">
            收藏
        </span>
    </div>`;
    }
    ...
    //添加关注动作
    document.getElementsByName("fav").forEach((elem)=> {
...
function onlyFavChange() {
    console.log('onlyFavChange');
    if (!document.getElementById('onlyFav').checked) {
        console.log('onlyFavChange no checked');
        return
    }
    ...
    document.getElementById("imageContainer").innerHTML = html;
    //添加取消关注动作
    document.getElementsByName("cancelFav").forEach((elem)=> {
        elem.addEventListener('click', (event)=> {
...
function cardPageInit() {
    ...
    btn.addEventListener('click', search);
    document.getElementById('onlyFav').addEventListener('change', onlyFavChange);
    // 监听全局键盘事件
    document.addEventListener('keydown', (event) => {
        // 检查是否按下回车键（keyCode 13 或 key 'Enter'）
        if (event.key === 'Enter' || event.keyCode === 13) {
